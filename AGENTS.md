...
- **未確定の価格をコードやseedに書かない**。`price_status: 'fixed'|'tbd'|'hidden'` で制御し、表示は `PriceLabel` に一元化。確定済みは 月9,900円/浄12,870円/育毛1回17,600円 のみ。
  期間限定キャンペーン（2026-09-01〜10-31・新規限定・男性限定）の「カット＋シェービング＋選べるフェイシャル 通常9,900円→8,800円（税込）」は
  2026-09-22 にオーナーが確定・実施を確認済み。表示は `lib/campaign.ts` だけを情報源にし、終了日時を過ぎると自動で消える
...
（`scripts/threads/judge.py`、`0033_threads_t7.sql`）。下書きは `predicted_metric`・`predicted_vs_baseline`・`topic_tag`（トピックタグ1つ。候補と禁止は `scripts/threads/tags.md`、`0037_threads_topic_tag.sql`）が無いとDBが拒否する。
